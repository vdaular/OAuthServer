...
namespace IdentityServer.LdapExtension
{
        ...
        where TUser : IAppUser, new()
    {
        ...
        public TUser Login(string username, string password, string domain)
        {
            ...
            {
                var match = ParseUsername(username);
                if (match.Success)
                {
        ...
        static Match ParseUsername(string input)
        {
            // Expresión regular para coincidir con los diferentes formatos de nombres de usuario
            string regexPattern = @"^(?<domain>[^\\@]+)\\(?<username>[^\\@]+)|^(?<username>[^@]+)@(?<domain>[^@]+)$";
