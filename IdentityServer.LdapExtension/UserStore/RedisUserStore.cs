        ...
        where TUser : IAppUser, new()
    {
        ...
        public IAppUser FindByUsername(string username)
        {
            ...
            if (result.HasValue)
            {
                ...
                string message = $"The key {foundSubjectIdKey} should not be existing or data is corrupted!";
        ...
        {
            const string keyBySubjectId = "IdentityServer/OpenId/subjectId/{0}@{1}"; // <== contains the full data
            const string keyByUsername = "IdentityServer/OpenId/username/{0}@{1}"; // <== contains a link to the SubjectId
            ...
            var subjectIdStorageKey = string.Format(keyBySubjectId, user.SubjectId, user.ProviderName);
            ...
            {
                string message = $"This data should not be already in redis. {string.Format(keyByProviderAndUserid, user.ProviderName, user.ProviderSubjectId)}";
                _logger.LogWarning(message);
            }
            ...
            rdb.StringSet(string.Format(keyByUsername, user.Username, user.ProviderName), subjectIdStorageKey); // Might cause issue... or hack...
