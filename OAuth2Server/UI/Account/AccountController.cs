...
namespace OAuth2Server.UI.Account
{
    /// <summary>
    ...
    [SecurityHeaders]
    [AllowAnonymous]
    public class AccountController(IConfiguration configuration,
        IIdentityServerInteractionService interaction,
        IClientStore clientStore,
        IAuthenticationSchemeProvider schemeProvider,
        IEventService events/*,
        ILdapUserStore users*/) : Controller
    {
        ...
        public async Task<IActionResult> Login(LoginInputModel model, string button)
        {
            ...
            //{
            //    var user = users.ValidateCredentials(model.Username, model.Password, model.Domain);
        ...
        private async Task<LoginViewModel> BuildLoginViewModelAsync(string returnUrl)
        {
            ...
            var configs = (ExtensionConfig)configuration.GetSection("ActiveDirectory").Get(typeof(ExtensionConfig));

            var domains = new List<string>();

            if (configs?.Connections.Count != null)
                domains = configs.Connections.Select(s => s.FriendlyName).ToList();
